  ...
  <div class="p-4 bg-white border border-slate-200 rounded-lg flex flex-col gap-3">
    <div class="flex gap-2 items-center">
      <component
        :is="icon"
        class="w-[20px] h-[20px]"
        :class="[colorClass, loading && 'opacity-20']"
      />
      <div class="font-normal text-gray-500">
        <span v-if="!loading">{{ label }}</span>
        <div v-else class="h-4 bg-gray-200 rounded w-20 animate-pulse"></div>
      </div>
    </div>
    ...
    <div class="flex gap-2 items-baseline">
      ...
      <span v-if="percentage !== undefined && !loading" class="font-normal text-gray-500"
        >({{ percentage }}%)</span
      >
      <div
        v-else-if="percentage && loading"

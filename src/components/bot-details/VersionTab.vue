...
const { dataPage, loading, searchTerm, rowsPerPage, fetchDataPage, debouncedFetch } =
  usePaginatedData<BotVersionItem, PageWithBotVersion<BotVersionItem>>(
    async (page, rows_per_page, search) => {
      if (props.botId) {
        const { data: response } = await API.botVersion.index(props.botId, {
          page,
          rows_per_page,
          search,
        })
...
const columns = ref<Column[]>([
  ...
  {
    header: t('bots.headers.completed'),
    key: 'completed',
    type: 'PROGRESS',
    tooltip: t('bots.completed_tooltip'),
  },
  {
    header: t('bots.headers.abandoned'),
    key: 'abandoned',
    type: 'PROGRESS',
    tooltip: t('bots.abandoned_tooltip'),
  },
...
const onChangeStatus = async (status: BotStatus) => {
  ...
  try {
    const { data: response } = await API.botVersion.changeStatus(
      props.botId,
    ...
    toast.add({
      ...
      detail:
        status === 'active'
          ? t('bot_details.version_activated')
          : t('bot_details.version_deactivated'),
      life: 3000,
    })
...
const onDelete = async () => {
  ...
  try {
    ...
    dataPage.value.data = dataPage.value.data.filter(
...
<template>
  <div class="flex flex-col gap-8 px-6 pb-6">
    <div class="flex gap-3">
      ...
      <Message v-if="!dataPage.meta?.has_active_version" severity="warn">
      ...
      <template #nameField="{ data }">
        <div class="flex">
          <div class="flex flex-col">
            <span class="block whitespace-nowrap max-w-[250px] truncate">
              {{ data.nameField.name }}
            </span>
            <span
              ...
              v-tooltip.bottom="{
                value: `${
                  $t('created_by', {
                    author: data.nameField.created.tooltip.author,
                  }) +
                  '\n' +
                  $t('created_at', {
                    date: data.nameField.created.tooltip.date,
                  }) +
                  (data.nameField.updated.tooltip.author
                    ? '\n\n' +
                      $t('updated_by', {
                        author: data.nameField.updated.tooltip.author,
                      }) +
                      '\n' +
                      $t('updated_at', {
                        date: data.nameField.updated.tooltip.date,
                      })
                    : '')
                }`,
            ...
            >
              {{
                $t('bots.last_updated', {
                  date: data.nameField.updated.label || data.nameField.created.label,
                })
              }}
            </span>
          </div>

...
const groupedTimeline = computed(() => {
  ...
  for (const item of timeline.value) {
    const dateKey = moment(item.kind === 'message' ? item.created_at : item.event_at).format(
      'YYYY-MM-DD',
    )
...
const getReply = (item: MessageItem) => {
  if (item.reply_to_message) {
    return {
      name:
        item.direction === 'inbound' ? props.contactName : props.assignedUser?.name || 'Unknown',
...
watch(
  ...
  async (newMessages, oldMessages) => {
    ...
    const newMessageReceived =
      oldMessages.length && newMessages.length && oldMessages[0].id !== newMessages[0].id
...
watch(
  () => messagesStore.lastDeletedMessage,
  (deleted) => {
    ...
    >
      ...
      >
        ...
        <template v-for="item in group.items" :key="item.id">
          ...
          <div v-else-if="item.kind === 'activity'" class="text-center my-8">
          ...
          >
            ...
            <div v-if="item.direction === 'outbound' && assignedUser" class="pl-4 flex">
              <Avatar

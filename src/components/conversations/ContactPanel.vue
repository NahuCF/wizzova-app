...
const {
  ...
  debouncedFetch,
  loadNextPage,
} = usePaginatedData<MessageItem>(async (page, rows_per_page) => {
...
const name = computed(() => (props.contact ? getContactName(props.contact) : undefined))
...
const jumpToMessageInChat = async (message: MessageItem) => {
  ...
  const result = await messagesStore.jumpToMessage(props.conversationId, page, positionFromEnd)
  if (!result) return
...
<template>
  <div v-if="contact && name" class="flex flex-col gap-6 h-full overflow-hidden">
    <div v-if="!isEditing && !isSearching" class="flex justify-between items-center p-6">
      ...
      <div>
        <Button severity="secondary" class="min-w-[32px]! h-[32px]!" @click="isEditing = true">
          <div>
            <IconEdit size="16" />
          </div>
        </Button>
    ...
    <div v-if="!isEditing && !isSearching" class="flex flex-col gap-6 p-6">
      ...
      <div
        ...
        v-if="contactFields.filter((f) => !f.is_primary_field).length > 0"
      >
      ...
      <div class="flex flex-col overflow-y-auto">
        ...
        <div v-if="!loadingMessages && messages.data.length === 0" class="flex justify-center p-8">
          <div class="text-center text-gray-400">
            {{ $t('conversations.no_messages_found') }}
          </div>
        </div>

        <div
          v-if="messages.data.length > 0 && !isLastPage && !loadingMessages"
    ...
    <div v-if="isEditing" class="flex flex-col gap-6 p-6">
      ...
      <ContactForm ref="contactForm" :fields="contactFieldStore.contactFields" :contact="contact" />
    </div>
  </div>
</template>

...
} = usePaginatedData<ConversationItem>(async (page, rows_per_page, search) => {
...
const conversations = computed<ConversationItem[]>(() => {
  return (
    conversationStore.conversationsByTab[currentTab.value].sort((a, b) => {
...
const selectConversation = (conversation: ConversationItem) => {
  ...
  if (focusSearch.value && searchType.value === 'message') {
    jumpToMessageInChat(conversation)
  }

  focusSearch.value = false
}

const togglePin = ({ conversation, pin }: { conversation: ConversationItem; pin: boolean }) => {
  if (pin) {
...
const jumpToMessageInChat = async (conversation: ConversationItem) => {
  if (conversation.matching_message) {
    const message = conversation.matching_message
    const result = await messagesStore.jumpToMessage(
      conversation.id,
      message.page,
      message.position_from_end,
    )
    if (!result) return
  ...
  <div class="flex flex-col bg-white h-full overflow-hidden">
    <div class="flex gap-3 p-6">
      ...
      <div v-show="!focusSearch">
        <Button @click="emit('onStartConversation')">
    ...
    <div v-else ref="conversationScroll" class="overflow-y-auto max-h-full" @scrollend="onScroll">
      <ConversationItem
    ...
    <div v-if="noItems" class="flex justify-center p-8">
      <div class="text-center text-gray-400">
        {{ $t('conversations.no_conversations_found') }}
      </div>
    </div>
    ...
    <ConversationFilters ref="filtersPopover" @onApply="onApplyFilters" @onReset="resetFilters" />
  </div>
</template>

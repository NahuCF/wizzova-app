...
<template>
  <Dialog
    ...
    class="min-w-[25rem] max-w-[1088px] w-full h-[668px]"
    ...
    <div class="flex h-full">
            ...
            >
              ...
              <template v-else>
                <div class="flex gap-[2px] justify-center items-center">
                  <component
                    v-for="(digit, idx) in String(i + 1).split('')"
                    :key="idx"
                    :is="digitIconMap[digit]"
                    size="18"
                    class="text-current"
                  />
                </div>
      ...
      <section class="flex flex-col justify-between w-full h-full px-8 py-6">
        ...
        <div class="flex justify-end gap-2 mt-6">
          ...
          <Button v-if="currentStep > 1 && currentStep !== 3" severity="secondary" @click="goBack">
            <span>
              {{ backLabel || 'Back' }}
            </span>
          </Button>

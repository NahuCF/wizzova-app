...
import { IconX, IconPlus, IconTrash, IconFilter } from '@tabler/icons-vue'
  ...
  const cleanedConditions = currentFilter.value.conditions
    .map((cond) => {
      ...
      const labels = selectedColumn.value?.options
        ? values.map(
            (v) => selectedColumn.value!.options!.find((opt) => opt.value === v)?.label ?? v,
          )
        : values
    ...
    .filter(
      (cond) =>
        cond.operator === 'is_empty' || cond.operator === 'is_not_empty' || cond.value.length > 0,
    )
  ...
  computed<Date | null>({
    ...
    set(newVal) {
      currentFilter.value.conditions[conditionIndex].value[valueIndex] =
        newVal instanceof Date && !isNaN(newVal.getTime()) ? newVal.toISOString().split('T')[0] : ''
    },
  })
</script>
    ...
    >
      <div class="flex justify-between gap-2 p-3">
        <div class="flex items-center gap-2">
          <component
            v-if="selectedColumn?.icon"
            :is="selectedColumn.icon"
            class="text-gray-500"
            size="14"
          />
        ...
        <template v-for="(condition, index) in currentFilter.conditions" :key="index">
          <div class="flex items-center gap-2 mb-3">
            <Select
              ...
              :options="getOperatorOptions(selectedColumn!)"
              optionLabel="label"
              optionValue="value"
              class="flex-1"
            />
          ...
          <div v-if="showValueInput(condition.operator)">
            <div
              ...
              v-for="(val, valIdx) in condition.value"
              :key="valIdx"
            >
              <!-- DATE -->
              <DatePicker

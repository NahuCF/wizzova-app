...
const props = defineProps<
  NodeProps & {
    ...
    data:
      | BotNodeDataMap['image']
      | BotNodeDataMap['video']
      | BotNodeDataMap['audio']
      | BotNodeDataMap['document']
  }
>()
...
const onSave = async () => {
  ...
  if (newFile.value?.data) {
    loading.value = true
    try {
      const { data: response } = await API.botVersion.uploadMedia(
        botId.value,
        newFile.value.data,
        newData.value.mediaType,
        props.id,
      )
    ...
    <div class="bg-white p-6 flex flex-col gap-8 overflow-y-auto overflow-x-hidden">
      <div>
        <div class="flex gap-1 mb-2">
          <h2 class="font-medium text-lg">
            {{
              $t('bot_workflow.send_media.media_type') +
              ` (${$t('bot_workflow.send_media.max_file_size', defaultMBLimit[newData.mediaType])})`
            }}
          </h2>
        </div>
        ...
        <div class="flex gap-2 w-full">
          <div v-for="type in allowedTypes" class="flex items-center gap-2">
            <RadioButton v-model="newData.mediaType" :inputId="type" :value="type" />
      ...
      <div v-if="newData.mediaType !== 'document'">
        <div class="flex items-baseline gap-1 mb-2 text-gray-500">
          <h2 class="text-lg">
            {{ $t('bot_workflow.send_media.allowed_media_type') }}:
            {{ defaultSupportedFormats[newData.mediaType].join(', ') }}
          </h2>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-1 mt-4">
        <div class="flex items-center justify-between">
          ...
          <span class="text-sm text-slate-400"> {{ newData.content?.length || 0 }} / 1024 </span>
        </div>

...
const canSave = computed(() => {
  const parametersFilled = Object.values(newData.value.templateParameters).every(
    (value) => value.trim() !== '',
  )
...
const template = computed(() =>
  templateStore.templates?.find((t) => t.id === props.data.template_id),
)
  ...
  >
    <div class="flex-basis bg-white p-6 flex flex-col gap-2">
      ...
      <div v-else-if="template">
        <MessagePreview
          :header="
            !Array.isArray(template?.components.header) ? template.components.header.text : ''
          "

...
const effectiveFormats = computed(
	() => props.supportedFormats || defaultSupportedFormats
)
const effectiveLimit = computed(
	() => props.mediaMBLimit || defaultMBLimit
)
const accept = computed(
	() => effectiveFormats.value[props.mediaType].join(',')
)
const maxFileSize = computed(
	() => effectiveLimit.value[props.mediaType] * 1000 * 1000
)
	...
	>
		...
		<template #content="{ removeFileCallback }">
			<div v-if="(internalMediaUrl || showLink) && !newFile" class="flex flex-col gap-1 mt-4">
				...
				<InputText v-model="internalMediaUrl" class="w-full"

...
watch(drawerVisible, (visible) => {
  if (visible) {
    ...
    assigneeId.value =
      props.data.assign_type === 'user' ? props.data.assign_to_user_id : props.data.assign_to_bot_id
  }
})
</script>
      ...
      <div>
        ...
        <div class="flex gap-2 w-full">
          <div class="flex items-center gap-2">
            <RadioButton v-model="assignType" inputId="user" value="user" />
            <label for="user" class="text-lg font-normal cursor-pointer">
              {{ $t('bot_workflow.assign_chat.user') }}
            </label>
          </div>
          <div class="flex items-center gap-2">
            <RadioButton v-model="assignType" inputId="bot" value="bot" />
      ...
      <div>
        <div class="flex gap-1 mb-2">
          <h2 class="font-medium text-lg">
            {{
              assignType === 'user' ? $t('bot_details.select_user') : $t('bot_details.select_bot')
            }}
          </h2>
        </div>

...
const groupedItems = computed(() => {
	...
	const groups: Record<BotNodeCategory, BotNodeItem[]> = {} as Record<BotNodeCategory, BotNodeItem[]>
...
<template>
	<aside class="max-w-[400px] w-full bg-white p-6 flex flex-col gap-4">
		...
		<div v-for="(items, category) in groupedItems" :key="category" class="flex flex-col gap-4">
			<div class="text-lg text-slate-700 uppercase">{{ $t(`bot_workflow.categories.${category}`) }}</div>

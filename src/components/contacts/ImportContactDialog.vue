...
const nextLabel = computed(() => {
  switch (currentStep.value) {
    case 1:
      return t('contacts.import_dialog.map_columns')
    case 2:
      return t('contacts.import_dialog.map_columns')
    default:
      return t('contacts.import_dialog.confirm_import')
  }
})

const isNextDisabled = computed(() => {
  return (
    (currentStep.value === 1 && !mappingRows.value.length) ||
...
const validateMappings = () => {
  ...
  if (!mappedLower.includes('name')) {
    validationErrors.value.push(
      t('contacts.import_dialog.required_field_missing', { field: 'name' }),
    )
  }
  if (!mappedLower.includes('phone')) {
    validationErrors.value.push(
      t('contacts.import_dialog.required_field_missing', { field: 'phone' }),
    )
  }
...
const submitImport = async () => {
  ...
  try {
    ...
    await API.contact.importContacts(
      file,
      renamedFileName.value || originalName,
      importMode.value,
      mappings,
    )
    success = true

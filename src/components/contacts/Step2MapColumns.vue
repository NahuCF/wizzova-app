...
<template>
    <div>
                ...
                <template #body="slotProps">
                    <Tag :severity="slotProps.data.status === 'MAPPED' ? 'success' : 'danger'" size="small" rounded>
                        <div class="flex items-center gap-2">
                            ...
                            <div>{{ t(`contacts.import_dialog.${slotProps.data.status.toLowerCase()}`) }}</div>
                        </div>
                    </Tag>
                </template>
            </Column>
        ...
        <Message v-for="error in validationErrors" :key="error" severity="error" :closable="false" class="my-2">
            {{ error }}
        </Message>
    </div>
</template>

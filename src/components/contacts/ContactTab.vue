...
const {
  ...
  (page, perPage, search) =>
    API.contact.index(page, perPage, search, flattenFilters(filters.value)).then((res) => res.data),
  10,
)
...
const handleActiveCampaign = (error: unknown) => {
  if (
    ...
    error.response?.data.message_code === 'contact_has_active_broadcasts'
  ) {
...
watch(rowsPerPage, () => fetchDataPage(1, rowsPerPage.value), { immediate: true })

watch(filters, () => fetchDataPage(1, rowsPerPage.value), { deep: true })
  ...
  <div class="flex flex-col gap-6 h-full">
    <div class="flex justify-between py-2">
      <div class="flex gap-2">
        <GenericFilters :columns="contactFilters" v-model:filters="filters" />
        <Button
      ...
      <template v-for="(filter, fIndex) in filters" :key="fIndex">
        <template v-for="(condition, cIndex) in filter.conditions" :key="cIndex">
          <Tag class="px-3!" rounded>
            ...
            <Button variant="text" rounded class="p-0.5!" @click="removeFromFilter(fIndex, cIndex)">
              <IconX class="w-4 h-4" />
            </Button>
          </Tag>
        </template>
    ...
    <ContactDrawer
      ...
      @onConfirm="onSave"
    />
    <ImportContactDialog v-model:visible="showImportContacts" />
    <WarningDialog

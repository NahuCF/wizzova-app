...
const resolver = zodResolver(
  z.object({
    ...
    internal_name: z.string().refine((value) => value.length > 0, {
  ...
  () => {
    if (props.visible && props.contactField) {
      ...
      if (props.contactField.options) {
        options.value = [...props.contactField.options]
      }
      internalNameEdited.value = true
  ...
  >
    ...
    >
      ...
      <div class="flex flex-col gap-1 relative">
        ...
        <Message v-if="$form.internal_name?.invalid" severity="error" variant="simple">
      ...
      <div class="flex flex-col gap-1 relative">
        ...
        <Select v-model="contactField.type" :options="types" />
      </div>
      ...
      <div v-if="contactField.type === 'SELECT'">
        <div class="flex flex-col gap-1 relative">
          ...
          <div
            ...
            :key="`field_option_${index}`"
            class="flex gap-5 items-center"
          >
            <InputText v-model="options[index]" fluid />
            <IconX
              @click="popOption"
              :class="{

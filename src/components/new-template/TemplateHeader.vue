...
<template>
  <div>
    ...
    <div class="flex flex-wrap gap-4">
      <div
        v-for="headerType in fullHeaderTypes"
        :key="headerType.id"
      ...
      >
        ...
        />
        <label class="text-xl cursor-pointer" :for="headerType.code">{{
          $t(`header_type.${headerType.code}`)
        }}</label>
      </div>
    </div>
    ...
    <div v-if="type === 'TEXT'" class="relative mt-3">
      ...
      />
      <div class="absolute right-3 top-2 text-slate-400">{{ text.length }} / 60</div>
    </div>
  </div>
</template>

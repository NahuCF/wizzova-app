...
const connectWaba = async () => {
  try {
    const authResponse = await launchLogin(
      'business_management,whatsapp_business_management,whatsapp_business_messaging,whatsapp_business_manage_events ',
    )
    if (!authResponse) {
...
const selectDefaultWaba = async () => {
  ...
  try {
    loading.value = true
    const { data: response } = await API.tenant.storeDefaultWABA(
      selectedBusiness.value.id,
      selectedWaba.value.id,
    )
          ...
          <div class="flex flex-col items-center gap-8 max-w-[400px] w-full">
            ...
            <div class="flex flex-col items-center gap-8 w-full px-12">
              <div class="flex flex-col gap-1 justify-center w-full">
                <label class="text-lg" for="wabaNumbers">{{
                  $t('complete_profile.phone_number')
                }}</label>
                <Select
              ...
              <div class="flex flex-col gap-1 justify-center w-full">
                <Button class="text-lg font-medium" @click="sessionStore.createNumber = true">

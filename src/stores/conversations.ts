...
export const useConversationsStore = defineStore('conversations', () => {
  ...
  const tabToFilters = (tab: ConversationStatus) => {
    switch (tab) {
      case 'unassigned':
        return { only_unassigned: true }
      case 'mine':
        return sessionStore.user ? { user_id: sessionStore.user.id } : {}
      case 'pinned':
        return { only_pinned: true }
      case 'opened':
        return { only_opened: true }
      case 'resolved':
        return { only_solved: true }
    }
  }
  ...
  const fetchConversations = async (
    ...
    try {
      ...
      conversationsByTab.value[tab] =
        page === 1 ? response.data : [...conversationsByTab.value[tab], ...response.data]
  ...
  const pin = async (conversation: ConversationItem) => {
    optimisticUpdate({
      ...
      rollback: () => {
        conversationsByTab.value.pinned = conversationsByTab.value.pinned.filter(

				...
				<Form v-slot="$form" class="flex flex-col gap-5" :initialValues="form" :resolver @submit="onFormSubmit">
					...
					<div class="flex flex-col gap-1 relative">
						...
						<div class="flex gap-2">
							<div @click="cellphonePopover.toggle($event)"
								class="flex items-center border border-slate-300 rounded-md p-2 hover:cursor-pointer">
								<img :alt="`Country flag selected imagen ${selectedCountry}`"
					...
					<div class="flex flex-col gap-1 relative">
						...
						<InputText v-model="form.workEmail" name="workEmail" id="workEmail"
							:placeholder="$t('business').toLowerCase() + '@email.com'" />
						<Message v-if="$form.workEmail?.invalid" severity="error" size="small" variant="simple"
							class="absolute bottom-[-1.4rem]">{{ $t($form.workEmail.error?.message) }}</Message>
					</div>
					<div class="flex flex-col gap-1 relative">
						...
						<Password v-model="form.password" class="w-full" placeholder="********" name="password"
							...
							<template #content>
								<ul class="pl-2 ml-2 my-0 leading-normal">
									<li class="flex gap-1 items-center" v-for="(rule, index) in passwordRules"
										:key="index">
										<IconCircleX class="text-red-500" size="22" v-if="
						...
						<Message v-if="$form.password?.invalid" severity="error" size="small" variant="simple"
							class="absolute bottom-[-1.4rem]">{{ $t('invalid_password') }}</Message>
					</div>

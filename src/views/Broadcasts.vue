...
const {
  ...
  debouncedFetch,
  onPage,
} = usePaginatedData<BroadcastItem>((_, perPage, search) => {
...
const tourSteps = computed((): Step[] => [
  {
    target: broadcastTitle.value,
    text: 'This is the broadcasts page.',
    position: 'right',
    highlight: true,
  },
  { target: newBroadcastBtn.value?.$el, text: 'Create a new broadcast here.', position: 'left' },
  {
    target: overviewSection.value,
    text: 'Your broadcast statistics live here.',
    highlight: true,
  },
  ...
  <div class="flex flex-col gap-6 h-full p-6">
    <div class="flex justify-between items-center">
      <div class="flex items-center">
        <h1 ref="broadcastTitle" class="font-semibold text-2xl">
          {{ $t('broadcasts.title') }}
        </h1>
      </div>
    ...
    <div
      ref="overviewSection"
      class="flex flex-col gap-5 p-6 bg-slate-200 rounded-lg border border-gray-300"
    >

...
const columns: Column[] = [
  ...
  {
    header: t('contact_imports.headers.errored_contacts'),
    key: 'errorContactsCount',
    type: 'PROGRESS',
  },
      ...
      <template #createdAtFormatted="{ data }">
        <div class="flex">
          <span
            v-tooltip.bottom="{
              value: `${$t('imported_by', {
                author: data['createdAtFormatted'].tooltip.author,
              })}\n${$t('imported_at', {
                date: data['createdAtFormatted'].tooltip.date,
              })}`,

...
const {
  ...
  (page, rows_per_page, search) =>
    API.bot.index({ page, rows_per_page, search: search || undefined }).then((res) => res.data),
  10,
)
...
<template>
  <div class="flex flex-col gap-8 p-6">
      ...
      <template #triggers="{ data: { triggers } }: { data: { triggers: BotTriggerTag[] } }">
        <div class="flex items-center gap-3">
          <template v-for="(trigger, index) in triggers" :key="`trigger-${index}`">
            <div v-if="trigger.tooltip" v-tooltip.bottom="trigger.tooltip">
    ...
    <CloneBotDialog v-model:visible="showCloneWarning" @onConfirm="onClone" />

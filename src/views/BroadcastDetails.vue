...
const {
  ...
  fetchDataPage,
  onPage,
} = usePaginatedData<MessageItem>((page, perPage) => {
  const broadcastId = route.params.id
  if (typeof broadcastId !== 'string')
    return new Promise((resolve) => resolve({ data: [], meta: {} }))
  ...
  <div class="flex flex-col gap-8 h-full p-6">
        ...
        <div v-else-if="broadcast.scheduled_at">
          ...
          <div class="flex items-center gap-2">
            <div class="font-medium text-gray-400">
              {{ moment(broadcast.scheduled_at).format('DD MMM YY, hh:mm A') }}
            </div>
            <div>
              <Tag
      ...
      <div>
        <div class="flex gap-2">
          <Button
            ...
            @click="() => refreshData()"
          >
            <IconRefresh v-if="loading" class="animate-spin" size="16" />
          ...
          <Button :disabled="true" @click="() => {}">
            <IconLoader2 v-if="loading" class="animate-spin" size="16" />
    ...
    >
      <template #name="{ data }: { data: MessageItem }">
        <div class="flex items-center gap-2">
          <Avatar :label="'Max Power'.charAt(0).toLocaleUpperCase()" shape="circle" />
          {{ 'Max Power' }}
        </div>
      </template>
      ...
      <template #status="{ data }: { data: MessageItem }">
        <div class="flex items-center gap-2">
          <component
            :is="statusIcon[data.status].icon"
            class="w-5 h-5"
            :class="statusIcon[data.status].class"
          />
